...
import top.cookie.pack.loader.DirectoryPackLoader;
import top.cookie.pack.loader.PackLoader;
import top.cookie.pack.loader.ZipPackLoader;
...
public class PackManager implements Closeable {
    ...
    public void loadPacks(Path directoryPath) {
        ...
        log.info("succeed load " + manifestMap.size() + " packs");
    }
    ...
    private PackLoader getLoader(Path path) throws IOException {
        ...
        if (loader == null) {
            log.warn("cloudburst.resources.unknown-format: " + path);
        }
        return loader;
    }
    ...
    public synchronized void closeRegistration() {
        ...
        boolean mustAccept = (boolean) Server.getInstance().getServerSets().get("force-resources");
